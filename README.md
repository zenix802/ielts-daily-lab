...
| **听力** | 6 套原创听力（Test 1 整套 40 题 + 2 套 Section 1 专项），浏览器朗读。做完题看带答案标记的原文，再**逐句精听**：一句一句听写、自动对照、跟读、记错因 |
| **阅读** | 原创学术类 Test 1：3 篇、40 题，题型和真题一致（判断、段落标题、单选、填空、句子配对）。限时作答，交卷后每题都有**原文依据 + 同义替换**，一点就在原文里高亮 |
...
![听力交卷后](docs/listening.png)
![逐句精听](docs/dictation.png)
![阅读解析](docs/reading.png)
...
- 听力用**浏览器自带的语音合成**朗读，男女声用两个英式声音区分（设备上只有一个英式声音时用音高区分）。机器朗读比真人录音清楚、没有口音和背景音，分数会偏高 —— 拿它练流程、拼写和逐句精听，检查点的成绩以真题为准。
...
下载 [`index.html`](index.html)，双击用浏览器打开。整个工具就这一个文件（约 300 KB），可以放 U 盘里带走。
